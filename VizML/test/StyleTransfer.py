...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    for layer in cnn.children():
        if isinstance(layer, nn.Conv2d):
            i += 1
            name = 'conv_{}'.format(i)
            load_name = "vgg19_conv_{}".format(j)
        elif isinstance(layer, nn.ReLU):
            name = 'relu_{}'.format(i)
            load_name = "vgg19_relu_{}".format(j)
            ...
            name = 'pool_{}'.format(i)
            load_name = "vgg19_pool_{}".format(j)
        elif isinstance(layer, nn.BatchNorm2d):
            name = 'bn_{}'.format(i)
            load_name = "vgg19_bn_{}".format(j)
        else:
            raise RuntimeError('Unrecognized layer: {}'.format(layer.__class__.__name__))
        j += 1
        load_model = torch.load("./vgg19/" + load_name + ".pt").to(device).eval()
        model.add_module(name, load_model)
...
def image_style_transfer_main(style_input_path, content_input_path):
        ...
        save_image(output_image, fp="./test.jpg")

image_style_transfer_main("./death.jpg", "./dancing.jpg")